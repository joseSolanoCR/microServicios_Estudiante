...
public class EstudianteControlador {
	...
	public ResponseEntity<?> actualizarEstudiante(@RequestBody Estudiante datosEstudiante, @PathVariable Integer id){
		try {
			Estudiante estudianteExiste = servicio.obtenerEstudianteById(id);
			if (estudianteExiste != null) {
				datosEstudiante.setId(id);
				Estudiante estudianteGuardado = servicio.guardarEstudiante(datosEstudiante);
				return new ResponseEntity<>(estudianteGuardado, HttpStatus.OK);
				}else {
					return new ResponseEntity<>("Estudiante no existe " + id, HttpStatus.NOT_FOUND);
				}
		} catch (Exception e) {
